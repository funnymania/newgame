...
// todo: how to displace the view_move, such that (0,0) is at its center
       ...
       SimpList<Polyhedron> polyhedra, GameMemory* game_memory, GameState* game_state)
{
    ...
    v3f64 world_rotation_now = NormalizedVector({ 0, 1, 0 }); 
...
// final_inputs has the RESULT of this... which is 
internal void ProcessInputsAndStage(GameOffscreenBuffer *buffer, std::vector<AngelInput> inputs, GameState* game_state, 
...
//                     is finished playing, but instead wait to check something on the game's timestep. In that case,
